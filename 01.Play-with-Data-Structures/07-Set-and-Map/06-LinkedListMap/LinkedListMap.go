...
// 传入一个key返回该节点的引用
func (lm *LinkedListMap) getNode(key K) *Node {
...
func (lm *LinkedListMap) Contains(key K) bool {
	return lm.getNode(key) != nil
}

func (lm *LinkedListMap) Get(key K) V {
	node := lm.getNode(key)
	if node == nil {
		return nil
	} else {
...
func (lm *LinkedListMap) Add(key K, value V) {
	node := lm.getNode(key)
	if node == nil {
...
func (lm *LinkedListMap) Set(key K, newValue V) {
	node := lm.getNode(key)
	if node == nil {
...
func (lm *LinkedListMap) Remove(key K) V {
	// RemoveElement代码逻辑修改
	prev := lm.dummyHead
	for prev.next != nil {
