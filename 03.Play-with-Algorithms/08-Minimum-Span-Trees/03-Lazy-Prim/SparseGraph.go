...
func NewSparseGraph(n int, directed bool) *SparseGraph {
	...
	// m: 0 表示初始化没有任何边
	return &SparseGraph{n: n, m: 0, directed: directed, g: make([][]*Edge, n)}
}

func (this *SparseGraph) V() int { return this.n } // 返回节点个数
func (this *SparseGraph) E() int { return this.m } // 返回边的个数
func (this *SparseGraph) AddEdge(v, w int, weight weight) { // 向图中添加一个边
	assert(v >= 0 && v < this.n)
	assert(w >= 0 && w < this.n)
	this.g[v] = append(this.g[v], NewEdge(v, w, weight)) // 在v中添加w,表示v和w相连
	if v != w && !this.directed {                        // 不是自环边并且是无向图
		this.g[w] = append(this.g[w], NewEdge(w, v, weight)) // 在w中添加v,表示w和v相连
	}
	this.m++ // 添加边的数目
...
func (this *SparseGraph) Show() {
	for i := 0; i < this.n; i++ {
		...
		for j := 0; j < len(this.g[i]); j++ {
			fmt.Print("( to:", this.g[i][j].Other(i), ", wt: ", this.g[i][j].Wt(), ")\t")
		}
		fmt.Println()
	}
}
...
// 返回图G中与顶点v相连接的第一个顶点
func (this *AdjIteratorSparseGraph) Begin() *Edge {
	this.index = 0
...
// 返回图G中与顶点相连接的下一个顶点
func (this *AdjIteratorSparseGraph) Next() *Edge {
	this.index++
