...
// 查找key的前驱节点
// 如果不存在key的前驱(key不存在,或者key是整棵二叉树的最小值),则返回null
func (bst *BST) predecessor(key int) int {
	node := search(bst.root, key)
	...
	preNode := predecessorFromAncestor(bst.root, key)
	if preNode == nil {
		return -9999
	} else {
		return preNode.key
...
// 算法调用前已保存key存在在以node为根的二叉树中
func predecessorFromAncestor(n *node, key int) *node {
	if n.key == key {
		return nil
	}
	...
	if key < n.key {
		...
		// 向下搜索到的结果直接返回
		return predecessorFromAncestor(n.left, key)
	} else {
		// 如果当前节点小于key,则当前节点有可能是比key小的最大值
		// 向右继续搜索,将结果存储到tempNode中
		tempNode := predecessorFromAncestor(n.right, key)
		if tempNode != nil {
			return tempNode
		} else {
...
func (bst BST) successor(key int) int {
	node := search(bst.root, key)
	// 如果key所在的节点不存在,则key没有前驱,返回null
	if node == nil {
		return -9999
...
// 算法调用前以保证key存在以node为根的二叉树中
func successorFromAncestor(n *node, key int) *node {
	if n.key == key {
		return nil
	}
	...
	if key > n.key {
		...
		// 向下搜索到的结果直接返回
		return successorFromAncestor(n.right, key)
	} else {
		// 如果当前节点大于key,则当前节点有可能是比key大的最小值
		// 向左继续搜索,将结果存储到tempNode中
		tempNode := successorFromAncestor(n.left, key)
		if tempNode != nil {
			return tempNode
		} else {
			// 如果tempNode为空,则当前节点即为结果
