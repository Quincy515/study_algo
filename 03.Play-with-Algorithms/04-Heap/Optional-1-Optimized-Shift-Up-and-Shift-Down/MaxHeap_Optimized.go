...
// 将ShiftUp和ShiftDown函数使用类似插入排序算法的方式进行优化的最大堆
type MaxHeap struct {
	data     []int
	count    int
	capacity int
...
// 该构造堆的过程,时间复杂度为O(n)
func NewHeapify(arr []int, n int) *MaxHeap {
	data := make([]int, n+1)
	capacity := n
	for i := 0; i < n; i++ {
		data[i+1] = arr[i]
	...
	count := n
	maxHeap := &MaxHeap{data, capacity, count}
...
func (h *MaxHeap) shiftUp(k int) {
	...
	for k > 1 && (h.data[k/2] < e) {
		h.data[k] = h.data[k/2] // parent(i)=i/2
		k /= 2
	}
	h.data[k] = e
}
...
func (h MaxHeap) shiftDown(k int) {
	...
	for 2*k <= h.count { // 完全二叉树中有左孩子，就有孩子
		...
		h.data[k] = h.data[j] // 不满足二叉堆定义,就交换k和左右孩子中较大的孩子
