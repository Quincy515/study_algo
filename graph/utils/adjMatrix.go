...
func NewAdjMatrix(name string) (*AdjMatrix, error) {
	...
	for i := 0; i < 2*E; i++ {
		if i%2 == 0 {
			...
			validateMatrixVertex(a, V) // 判断顶点是否合法
			b, _ := strconv.Atoi(result[3+i])
			validateMatrixVertex(b, V)
...
func (a *AdjMatrix) HasEdge(v, w int) bool {
	validateMatrixVertex(v, a.V)
	validateMatrixVertex(w, a.V)
	return a.adj[v][w] == 1
}

// 返回v顶点的相邻的边
func (a *AdjMatrix) Adjacency(v int) []int {
	validateMatrixVertex(v, a.V) // 判断用户传入的顶点v是合法的
	res := make([]int, 0)
	for i := 0; i < a.V; i++ {
		if a.adj[v][i] == 1 {
			res = append(res, i)
...
func validateMatrixVertex(v, V int) {
	if v < 0 || v >= V {
