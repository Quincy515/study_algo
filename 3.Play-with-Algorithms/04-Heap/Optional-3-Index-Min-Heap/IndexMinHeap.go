...
// 索引堆中,数据之间的比较根据data的小小进行比较,但实际操作的是所有
...
func (h IndexMinHeap) shiftDown(k int) {
	for 2*k <= h.count { // 完全二叉树中有左孩子，就有孩子
		...
		swap(h.indexes, k, j) // 不满足二叉堆定义,就交换k和左右孩子中较小的孩子
		h.reverse[h.indexes[k]] = k
...
// 传入的i对用户而言,是从0索引的
func (h *IndexMinHeap) Insert(item int, i int) {
...
func (h *IndexMinHeap) ExtractMin() int {
	if h.count <= 0 {
		panic("堆中没有元素!")
	}
...
// 获取最小索引堆中的堆顶元素
func (h *IndexMinHeap) GetMin() int {
	if h.count <= 0 {
		panic("堆中没有元素!")
	}
...
// 获取最小索引堆中索引为i的元素
func (h *IndexMinHeap) GetItem(i int) int {
	if !h.contain(i) {
		panic("该索引没有元素!")
	}
	return h.data[i+1]
...
// 将最小索引堆中索引为i的元素修改为newItem
func (h *IndexMinHeap) change(i int, newItem int) {
	if !h.contain(i) {
		panic("该索引没有元素!")
	}
	i += 1
